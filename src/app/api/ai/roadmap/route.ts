...
function errorResponse(error: unknown) { if (error instanceof AiContextError || error instanceof AiServiceError) return NextResponse.json({ error: error.message, code: error.code }, { status: error.code === "AI_NOT_CONFIGURED" ? 503 : 422 }); return NextResponse.json({ error: "Unable to generate a roadmap right now." }, { status: 500 }); }
